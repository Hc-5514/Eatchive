...
| --- | --- | --- |
| Daily quota | 25,000 requests/day per app for Search API (covers local search & manual fallback). citeturn0search4 | 100,000 requests/day per keyword search/geocoding endpoint under free tier. Paid upgrade available via Kakao Paid API. citeturn0search0turn0search11 |
| Authentication | Client ID/Secret headers; per-app quotas; simpler onboarding but lower cap. | REST API key; free quota generous but requires Kakao Map activation + Paid API enablement for scaling, more onboarding steps. citeturn0search11 |
...
| Cost | Free within 25k/day; beyond requires commercial arrangement (out of MVP scope). | Free within 100k/day; paid tier billed per call for higher volumes. citeturn0search0 |
...
## Detailed Analysis
### Quotas & Scaling
- **Naver**: Search API (including local search) limited to 25,000 calls/day per application. Monitor daily usage via planned Prometheus counter; alert at 80% threshold. citeturn0search4
- **Kakao**: Local API endpoints (keyword search, geocode) provide 100,000 free calls/day; exceeding free tier requires enabling Paid API and billing wallet. citeturn0search0turn0search11
...
- **Kakao**: Requires Kakao Dev app with Map APIs enabled and Paid API toggle for higher tiers. Additional compliance overhead; keep as contingency only. citeturn0search11
